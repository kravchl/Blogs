...
Usable security is a hot topic these days. The paper went over some helpful refreshers that I've studied in the past in regards to security a network from a basic technical understanded. Password policies, encryption, proper authentication, and similar security principles need to be the primary design structure of any system. These days we're busy playing catchup to the swiss cheese systems that we're designed with user interface accessablity and very little security. Fixing these systems, as the paper points out, can be difficult if not pointless. In many cases starting from the ground up is necessary if systems are to be survive in the cyberwar hacker climate we find ourselves in.
...
Artificial intelligence is humankinds means to a more advanced future. The human mind is painfully slow at learning and in order for major advancements in technology to occur, we need to develop all of our efforts into this technology. A physicist can spend twenty or so years learning how and why the world works before even attempting to develop any new theories. Similarly, a person can devote the same amount of time on mastering a game like Go, before attempting to create game breaking new strategies. A properly coded self-learning AI system can potentially learn both in a matter of months if not less. There’s no doubt that AI will be used to gain an advantage in cyberwarfare between nations, however in order to create the level of trust needed to safely consider implementing it, the development and design needs to revolve around security.

## Introduction

The artificial intelligence field has been advancing faster than expected in the past decade and with it comes a growing need to consider the repercussions of such a powerful form of technology. It’s human nature to want to find the next big leap in development that is stronger, bigger, and better than before. If one looks at the history of nuclear development, it’s clear to see that some people didn’t fully understand what potential dangers were involved in managing nuclear products. If we handle artificial intelligence like we do a potential nuclear reactor explosion (very cautiously), then maybe there is hope for the human race. Hastily made decisions involving a militarization of technology can result in a chain of catastrophic events which can lead to deaths and fuel tensions between nations. Countries will always have differences of opinions and unfortunately war is unavoidable. The biggest threat now and arguably the biggest threat ever to the safety of humans everywhere is the advancement of AI and its potential applications in robot warfare.

## What is AI? 

Very quickly computer scientists discovered that if AI is to come into fruition, that attempting to program every possible scenario within a given task leads to a dead end in available computing resources. The first demonstratable use of AI was back in 1951, used to master the tasks involved in checkers [1]. Since then the system has improved on and mapped out every possible game board setup that could follow after a player moves a single piece. It’s hard to fully grasp, but there exists 10 to the 31 possible setups [1]. 
However, this system cannot be expanded on to any game more complicated such as chess as the number of game boards increases to 10 to the 123 [1]. The sheer computational power needed to map and traverse every possible game in the tree is too great for modern technology. So, computer scientists needed to find a way to mimic learning the game through trial and error. Much like the human learning process, which relies on repeated exposure to a single given task, machines were programmed to follow the same process of learning. 
The design of trial and error is simple in principle but complicated in the coding necessary to achieve it. With chess, the system had to be given time to “see” or store matches played by master level chess players. Once the games were stored, they were analyzed play by play and given probabilities of success rates [1]. In other words, moving a piece to a specific place on the board given the current board layout has a certain probability of resulting in a win [1]. Of course, this type of probability reasoning requires mass amounts of known moves in order to be successful. 
The beauty of computers is their ability to perform simple tasks near instantaneously. So, one can imagine how quick this chess system can play through a single game against itself. Simply let the machine play against itself for millions of iterations, and you will get a near unbeatable system. This principle has since been successfully applied by AI company DeepMind to the game of Go, which has 10 to the 360 game boards in its tree, as well as more intricate games like poker [3]. What used to be considered hundreds of years away from being developed has been conquered.
If we consider the conquering of these games as steppingstones in the development of AI, we can quickly see where such a technology could be applied to. These AI gaming systems give us important insight into the potential power and risks involved in developing robotics. 

## How does conquering games translate?

While impressive, creating systems that are unbeatable in games of strategy don’t mean much for society. However, the potential of applying the same principles of “machine learning” can be applied to virtually any task. A similar approach was taken to teach a four limbed robot how to walk [2]. Not only did this robot learn how to walk, albeit in an unconventional and unexpected way, but it learned to do so without any information about its limbs, how to use them, and examples of how to use them. In other words, this robot learned what its own body looked like, what actions created motions, and ultimately what actions needed to be done on its four limbs to move across the floor [2]. 
The same artificial intelligence algorithms that can be used to teach walking can be taught to perform any other human function. The current problems with robotics are the clumsiness of motion. The programs in place are too slow to respond to changes in the environment, so a robot may fall over something that a human would simply step over without a second thought. However, this processing delay is largely diminished the more a system learns about physical objects. In addition, the processing power of the system is also limiting, but with computing speeds improving every day and the eventual use of quantum computing, we’ll soon have systems that put the human brain’s processing ability to shame.

## Potential of Artificial Intelligence?

If one would ask the general population what they think AI is, they usually respond in a post-apocalyptic vision of the world with mankind fighting off omnipotent robots. Albeit the result of the entertainment industries’ spin on AI, this could very well be a reality in the nearby future [3]. With the advancements in robotics that can mimic nearly every motion a human is able to perform, including pulling a trigger on a gun [3], paired with a program that mimics human cognition, this robot could start to learn how to aim nearly perfect in just a few months or days with todays AI technology. Now, copy that program to however many more robots you need, and you have an army of death [3].
What’s even more scary is a robot that can learn to decide on its own, what targets are the enemy and are deemed a big enough threat to shoot. It may seem impossible to most people, but there are systems currently that are capable of what we understand to be learning. 
There is robotics used in surgery today to perform incredibly precise incisions in high risk procedures, that allow for the removal of human error. All it takes is a small cut in the wrong place to cause a person to bleed out on the operating table. While these systems do rely on a surgeon to handle the two robotic arms, it’s not too far off in the future where AI could be introduced to perform the surgery without any outside assistance. If robots can be taught to perform surgery, then firing off weapons is an easy task by comparison.

## Trust Issues within Technology

There is undoubtedly major distrust in the computer world. With computers being broken into regularly and major privacy breaches happening to once trusted companies, it will be an uphill battle implementing AI robotics into the medical field. There exists an immediate trust when a new technology that becomes available, that is more fragile than a trust relationship built over time between humans [3]. If a surgeon has performed ten dangerous surgeries with no complications, they have started to build this foundation of trust that allows people to be at ease that they’ll be fine going into surgery without ever speaking to the surgeon. Now if there was a robot that performed the same ten surgeries without complications, the same person may or may not trust the robot to perform their surgery. If robotic surgery was the norm, then of course the trust would be there, but it’s getting to that point where we embrace the technology and allow for it to become the norm that is difficult. Similarly, if we have a robot that’s successfully patrolled without killing an “innocent” person not involved in a hypothetical war, all it may take for people to completely lose trust in the systems is one malfunction where the robot goes AWOL and kills a person.
When designing regular software, with simple tasks, it’s far too common to release systems with bugs and vulnerabilities within the system. The idea that the software functions well enough and can be deployed with small issues is a notion shared by the tech industry largely due to the need of funding. Development costs money and depending on how complicated a system becomes; funds can run out faster than expected. So, we find these swiss cheese programs flocking into the public that quickly become abused by the inevitable slew of hackers attempting to make some money off of other’s inability to afford to secure a system.
This cannot be the method in which AI is released onto public affecting systems. There needs to be a level of security that is impenetrable with a failsafe in place to shut down the system. The worst future imaginable would be if one group of people were to create an all-powerful AI that could rule the world. As Elon Musk said, a leading expert in modern AI, “for an AI, there would be no death, we would have an immortal dictator.” [3] The key to developing these systems is to democratize them and ensure the impacts and dangers are considered and addressed before continuing. Or else we may find ourselves with an unstoppable force that infects every network accessible system on the planet.

## What are the subtle dangers in AI?

We have already discussed the dangers of killer robots and omnipotent AI dictators. It may not be clear how we would arrive at either point, but blindly developing systems that can learn is clearly one means to both scenarios. We no longer have to just consider a hacker or insider gaining access and wreaking havoc on the world. Now, the system itself is a threat. In the case of AI robots, this threat can mean deaths and not just a loss in a strategy game. The algorithms used in the beginning stages of a system’s “machine learning” are obviously understood by the programmers. However, when the system has gone through millions of iterations of self-learning, it reacts in ways that the programmers do not understand. In the system that conquered the game of Go, there were times where moves were played that made absolutely no sense to humans but ended in the system successfully winning the game [5]. It’s in this loss of understanding that the danger lies. A machine will perform what it has “learned” to do. If a machine somehow concludes that the easiest way to win a game of Go is to simply kill the opposing player, then there is nothing in the system’s database stopping it from doing so. There are no ethical codes or emotions involved. Of course, a system intended to play Go will not have the means to kill, but obviously, weaponized AI robots will.

## Conclusion
For AI to continue and be implemented in various forms, there needs to be a deep understanding of where the system’s knowledge is headed and how it arrived at that conclusion. Restrictions to the system is key in preventing apocalyptic realities where the AI system has infected computers and humans need to be eradicated. We must proceed with caution and create a foundation of trust that allows for developing robotics that can potentially be used to act out war without the risk of the systems “learning” unintended and potentially deadly things. 

### REFERENCES
[1]	Anon. AlphaGo: The story so far. Retrieved June 11, 2020 from https://deepmind.com/research/case-studies/alphago-the-story-so-far
[2]	Anon. Cognitive Robotics and Artificial Intelligence. Retrieved June 11, 2020 from https://www.swissnexsanfrancisco.org/event/cognitiverobotics/
[3]	Anon. Collective Information Practice: Exploring Privacy and Security as Social and Cultural Phenomena. Retrieved June 11, 2020 from https://www.tandfonline.com/doi/abs/10.1207/s15327051hci2103_2
[4]	freeCodeCamp.org. 2020. Want to know how Deep Learning works? Here's a quick guide for everyone. (March 2020). Retrieved June 2, 2020 from https://www.freecodecamp.org/news/want-to-know-how-deep-learning-works-heres-a-quick-guide-for-everyone-1aedeca88076/
[5]	Mark Latonero. 2018. Governing Artificial Intelligence. (October 2018). Retrieved June 2, 2020 from https://datasociety.net/library/governing-artificial-intelligence/
[6]	Unknown. 1970. Part 2: AlphaGo under a Magnifying Glass (The historic match of deep learning AlphaGo vs. Lee Sedol) REVISED. (January 1970). Retrieved June 11, 2020 from http://deeplearningskysthelimit.blogspot.com/2016/04/part-2-alphago-under-magnifying-glass.html
